...
| Gesture arbitration | Start a vertical scroll on a card before activation and prove that it scrolls without opening the editor; tap still opens Edit | Feel the 160 ms visual feedback, one activation haptic at 440 ms, 7 px cancellation tolerance, and lack of accidental drags during ordinary scrolling |
| Reordering | Move first-to-last and last-to-first across offscreen rows with edge autoscroll, verify each order, relaunch persistence, and collect frame statistics | Reject any visible stutter, twitch, missed insertion gap, excessive sway, or wrong clean-handoff settling |
...
| Backup/export | Import through Android's picker; export through Android's create-document UI; verify a file was written | Open the exported file independently if device-specific document-provider behavior differs |
